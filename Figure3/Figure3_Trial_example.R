...
# Read the data
df <- read.csv("Figure3/Trial_illustration_S5.2.csv")  # Replace with actual path if needed

## For visualization purposes, patients #28 and #29 were slightly offset along the x-axis to avoid overlap
df[which(df$ID==28),"t.enrollment"] = 413+6
df[which(df$ID==28),"t.treated"] = 413+6

df[which(df$ID==29),"t.enrollment"] = 419+3
df[which(df$ID==29),"t.treated"] = 419+3
...
ggsave("Figure3/Figure3_Trial_example.jpg", p, limitsize = FALSE, width = 10, height = 8, dpi = 1000)
getwd()
