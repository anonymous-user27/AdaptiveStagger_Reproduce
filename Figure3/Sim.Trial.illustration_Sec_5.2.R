...
#             
#  This file R code to replicate Trial illustration in Section 5.2, Figure 3 
#-----------------------------------------------------------------------------------------------------------------#
...
#---------------   Explaination     ----------------------#
# Due to randomness, you might encounter model.unsucc.idx == 1.
# This is not caused by an error in the code.
# The issue arises from the inherent randomness in JAGS.
# Simply rerun the code — it should work correctly on the next attempt.
#--------------------------------------------------------------------#

#------------------------------------------------------------------#
# Function to achieve one-time realization of Adaptive Stagger Design
#--------------------------------------------------------------------#
Adaptive_Stagger_design_onetime <- function(dose, startdose, max_n, cohortsize, 
...
#------------------------------------------------------------------#
# Settings for Trial Illustration in Section 5.2
#--------------------------------------------------------------------#
...
set.seed(2237)
Adaptive_Stagger_illustration <- Adaptive_Stagger_design_onetime(dose , startdose, max_n, cohortsize,
                                                DLT_window, efficacy_window,  stagger_window,
                                                pi_T_bar, pi_E_bar, eta_elicited_rate, utility.table,
                                                accrual.rate, arrive.dist,event.dist,
                                                lambda_d, rho,  pT.true, pE.true,
                                                ...
                                                alpha0_bar,tau_alpha0,alpha1_bar,tau_alpha1,alpha2_bar,tau_alpha2, ## prior parameter for mu_E
                                                alpha3_bar, tau_alpha3)
Adaptive_Stagger_illustration
## round the numbers
df <- Adaptive_Stagger_illustration[[1]]
cols_to_round <- grep("^t\\.|^(VT|VE)$", names(df), value = TRUE)
df[cols_to_round] <- lapply(df[cols_to_round], round)

write.csv(df,"Figure3/Trial_illustration_S5.2.csv", row.names = FALSE)
