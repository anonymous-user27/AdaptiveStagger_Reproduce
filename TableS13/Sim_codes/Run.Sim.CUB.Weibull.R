...
#             
#  This file R code to replicate Simulation results for "CUB" design in Table S13 
###################################################################################################################
...
source("Sim_codes_functions/Sim.func.CUB.R")
##scenarios
...
####       Data generation          ####
##six dose
#dose <- c(8e+6, 4e+7, 8e+7, 4e+8, 8e+8,4e+9)
...
File_name="CUB_Weibull"


s1<- Sys.time()
CUB_design_sce <- CUB_design(dose , startdose, max_n, cohortsize,
...
filename <-sprintf("Results/%s_ctox_%.2f_ceff_%.2f_ces_%.2f_scen_%d_subset_%d.RData",File_name, cut.tox,cut.eff, cut.escalation,sce,subset)
save(CUB_design_sce,file=filename)
