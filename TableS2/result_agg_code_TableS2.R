...
#             
#  This file R code to aggregate simulation results and get numbers presented in  Table S2 (No Efficacy Decay)
###################################################################################################################
...
Design = "Adaptive_Stagger_nodecay"
ndose = 5
Nsim=25
cut.tox <- c(0.1)
cut.eff <- c(0.05)
...
library(dplyr)
method_order <- c("Strict_Stagger_nodecay", "BOIN12_nodecay","Adaptive_Stagger_nodecay")
Combined_results <- Combined_results %>%
  mutate(Method = factor(Method, levels = method_order)) %>%
  arrange(Scenario, Method)

write.csv(Combined_results,"TableS2/Sim_results_no_eff_decay_TableS2.csv", row.names = FALSE)
