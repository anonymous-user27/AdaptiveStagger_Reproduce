...
#             
#  This file R code to aggregate simulation results and get numbers presented in  Table S3 (Quick Efficacy Decay)
###################################################################################################################
...
Design = "Adaptive_Stagger_quickdecay"
ndose = 5
Nsim=25
cut.tox <- c(0.1)
cut.eff <- c(0.05)
...
Design = "Strict_Stagger_quickdecay"
ndose = 5
Nsim=50
cut.tox <- c(0.1)
cut.eff <- c(0.05)
...
library(dplyr)
method_order <- c( "Strict_Stagger_quickdecay", "BOIN12_quickdecay","Adaptive_Stagger_quickdecay")
Combined_results <- Combined_results %>%
  mutate(Method = factor(Method, levels = method_order)) %>%
  arrange(Scenario, Method)

write.csv(Combined_results,"TableS3/Sim_results_quick_eff_decay_TableS3.csv", row.names = FALSE)
