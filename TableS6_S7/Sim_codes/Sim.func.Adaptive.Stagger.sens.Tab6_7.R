###################################################################################################################
#  Sensitivity analysis of Adaptive Stagger design with the rule  U(non-stagger) ≥ U(stagger)
#  Function to generate operating characteristics of the design. 
###################################################################################################################
...
source("Sim_codes/Get.patients.tox.eff.outcome.R")
source("Sim_codes/Response.decay.rate.func.R")
source("Sim_codes/Functions.R")
...
#' @param rho.time Correlation between time to DLT and time to response 
#'                 We recommend the default value of (\code{rho.time=0}) for general use
#-------------------------------------
...
Adaptive_Stagger_sens <- function(dose, startdose, max_n, cohortsize, 
                                    ...
                                    alpha3_bar, tau_alpha3, sce=1, subset=1,File_name="Adaptive_Stagger_sens", rho.time = 0){
  ...
  for(trial in 1:Nsim){
      ...
      if(npat_dose[current_dose] == 0){
        ...
        for(j in 1:cohortsize){
          ...
          if(j==1){
            ...
            pts.toxeff.out <- get.pts.tox.eff.outcome(d = current_dose, npts=1, mu_E, mu_T, Sigma, DLT_window, efficacy_window, dist=event.dist,rho.time)
            ...
            for(r in 1:num_stag_rule){
              ...
              jags.params<-c("lambda", "rho","mu_T", "mu_E")#  parameters of interest
              ...
              jags.inits <- list(
                list("gamma0" = 0,"gamma1"=0,"beta0"=0,"beta1"=0,"beta2"=0, "alpha0"=0,"alpha1"=0,"alpha2"=0,"alpha3"=0),
                list("gamma0" = 0,"gamma1"=0,"beta0"=0,"beta1"=0,"beta2"=0, "alpha0"=0,"alpha1"=0,"alpha2"=0,"alpha3"=0),
                list("gamma0" = 0,"gamma1"=0,"beta0"=0,"beta1"=0,"beta2"=0, "alpha0"=0,"alpha1"=0,"alpha2"=0,"alpha3"=0),
                list("gamma0" = 0,"gamma1"=0,"beta0"=0,"beta1"=0,"beta2"=0, "alpha0"=0,"alpha1"=0,"alpha2"=0,"alpha3"=0)
              )
              model.unsucc.idx = 0
              runjagsmodel <- tryCatch({run.jags(model =joint_model, monitor = jags.params,data = jags.data,
              ...
              if(sum(delta.tox) == length(delta.tox)){
                ...
                pts.toxeff.out <- get.pts.tox.eff.outcome(d = assign.dose, npts=1, mu_E = mu_E_delay, mu_T, Sigma, DLT_window, efficacy_window, dist=event.dist,rho.time)
                ...
                stagger.ind <- ifelse(utility.stagger > utility.non.stagger,1,0) 
        ...
        ##Need to get this patient's outcome 
        pts.toxeff.out <- get.pts.tox.eff.outcome(d = current_dose, npts=cohortsize, mu_E, mu_T, Sigma, DLT_window, efficacy_window, dist=event.dist, rho.time)
    ...
    if(model.unsucc.idx == 0){
      ### dose selection 
      sim.data <- data.frame(dose_each_patient=dose_each_patient, yT.true = yT.true, yE.true = yE.true, n.low.tox=n.low.tox, t.low.tox=t.low.tox,
                             idx_stagger = idx_stagger,npat_initial_dose=npat_initial_dose)
  ...
  percent_select <- c()
  for(d in 1:(1+ndose)){
    percent_select[d] <- sum(report_result$OBD==(d-1))/(Nsim - model.unsucc)  
  }
...
# 
# 
# Adaptive_Stagger_results <- Adaptive_Stagger_sens(dose , startdose, max_n, cohortsize,
#                                                 DLT_window, efficacy_window,  stagger_window,
#                                                 pi_T_bar, pi_E_bar, eta_elicited_rate, utility.table,
#                                                 accrual.rate, arrive.dist,event.dist,
#                                                 lambda_d, rho, pT.true, pE.true,
