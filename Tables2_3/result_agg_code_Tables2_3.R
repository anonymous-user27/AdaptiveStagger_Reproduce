...
#             
#  This file R code to aggregate simulation results and get numbers presented in  Tables 2 and 3
###################################################################################################################
...
library(dplyr)
method_order <- c( "CUB", "Strict_Stagger", "BOIN12","Adaptive_Stagger")
Combined_results <- Combined_results %>%
  mutate(Method = factor(Method, levels = method_order)) %>%
  arrange(Scenario, Method)

write.csv(Combined_results,"Table2/Sim_results_Tables2_3.csv", row.names = FALSE)
